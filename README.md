<!-- GitHub README (HTML 기반) -->

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:1e3c72,100:2a5298&height=200&section=header&text=KimDongHyuk%20GitHub🌱&fontSize=60&fontColor=ffffff&animation=twinkling"/>
</div>

<h2>🏆 Achievements</h2>
<div>
  <details>
    <summary>[2025. 05. 30] 한성대학교 캡스톤디자인 Qualcomm 기업연계 프로젝트 어플리케이션 부문 <strong>우수상</strong></summary>
    <img src="https://github.com/user-attachments/assets/446a897c-3d2d-4008-9b48-d496a4d1f5f0" width="400"/>
  </details>
</div>


<h2>👥 Team Projects</h2>
<p>2025.02 - 2025.05 한성대학교 캡스톤디자인 기업연계(Qualcomm) [팀:무무]</p>

<details>
  <summary><strong>온디바이스 지식그래프 기반 AI 지식 관리 앱 "BrainTrace" <a href="https://github.com/HS-MUMU/BrainTrace_OnDeviceAi?tab=readme-ov-file">MuMu</a></strong></summary>
  <br/>
  <img src="https://github.com/user-attachments/assets/3c98fd2c-b38e-4ede-9704-d518111129e1" width="750" alt="braintrace" />
  <h4>📝 프로젝트 소개</h4>
  <p><strong>BrainTrace</strong>는 PDF, 오디오, 텍스트 등 다양한 문서를 자동 분석해 지식 그래프를 구축하고, 이를 기반으로 문맥 이해형 질문 응답을 제공하는 <strong>온디바이스 AI 솔루션</strong>입니다.<br/>
  외부 전송 없이 로컬에서 처리되며, <strong>출처 기반 응답</strong>으로 신뢰성과 투명성을 동시에 제공합니다.</p>

  <h4>🌟 주요 기능</h4>
  <ul>
    <li><strong>온디바이스 AI:</strong> 외부 전송 없이 완전 로컬 처리, 추가 비용 없이 프라이버시 보장</li>
    <li><strong>출처 기반 응답:</strong> 핵심 노드(출처) 제공으로 신뢰성과 투명성 보장</li>
    <li><strong>Graph-RAG 기반 정밀 추론:</strong> 질문과 유사한 노드 탐색 → 관계 추론 → 구조화된 응답 생성</li>
    <li><strong>자동 문서 업로드 & 지식그래프화:</strong> PDF, 오디오, 텍스트 자동 분석 → 시각화 및 검색</li>
    <li><strong>노드 기반 문서 검색 기능:</strong> 키워드 입력 시 관련 노드 및 문서 유사도 기반 검색</li>
  </ul>

  <h4>🖥️ 시연 영상</h4>
  <a href="https://www.youtube.com/watch?v=Q0w4S_sMEaQ" target="_blank">
    <img src="https://img.youtube.com/vi/Q0w4S_sMEaQ/0.jpg" width="400"/>
  </a>

  <h4>🖥️ llama-3b(eng).ver Video</h4>
  <a href="https://www.youtube.com/watch?v=r6AdiX7SZkw" target="_blank">
    <img src="https://img.youtube.com/vi/r6AdiX7SZkw/0.jpg" width="400"/>
  </a>
</details>

<br/>

<div align="center">
  <h2>🛠️ Tech Stacks</h2>
  <img src="https://img.shields.io/badge/Java-007396?style=flat-square&logo=openjdk&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring Boot-6DB33F?style=flat-square&logo=Spring&logoColor=white"/>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=Python&logoColor=white"/>
  <img src="https://img.shields.io/badge/C++-00599C?style=flat-square&logo=C%2B%2B&logoColor=white"/><br/>
  <img src="https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=FastAPI&logoColor=white"/>
  <img src="https://img.shields.io/badge/Neo4j-008CC1?style=flat-square&logo=Neo4j&logoColor=white"/>
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=MySQL&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=Git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=GitHub&logoColor=white"/>
</div>

<br/>

<div align="center">
  <h2>🧑‍💻 Contact Me</h2>
  <a href="mailto:kdh367036@naver.com">
    <img src="https://img.shields.io/badge/Email-03C75A?style=flat-square&logo=Naver&logoColor=white"/>
  </a>
  <a href="https://devkdh.tistory.com/" target="_blank">
    <img src="https://img.shields.io/badge/Tistory-000000?style=flat-square&logo=Tistory&logoColor=white"/>
  </a>
</div>

<br/>

<div align="center">
  <h2>🏅 GitHub Stats</h2>
  <img src="https://github-readme-stats.vercel.app/api?username=Kimdonghyuk0&show_icons=true&theme=radical&hide_border=true&bg_color=00000000"/>
  <br/>
  <img src="https://github-readme-streak-stats.herokuapp.com?user=Kimdonghyuk0&theme=radical&hide_border=true&background=00000000"/>
  <br/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Kimdonghyuk0&layout=compact&theme=radical&hide_border=true&bg_color=00000000"/>
</div>
